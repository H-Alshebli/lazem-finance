...
function buildHtml({ title, body, requestTitle, statusLabel, actorName }) {
  return `
    <div style="font-family:Arial,sans-serif;background:#f6f8fb;padding:24px;color:#111827">
      <div style="max-width:620px;margin:auto;background:#ffffff;border-radius:14px;border:1px solid #e5e7eb;overflow:hidden">
        ...
        <div style="padding:22px">
          ...
          <p style="font-size:12px;color:#6b7280;margin-top:22px">Please open the Lazem Finance Portal to review or take action.</p>
        </div>
      </div>
    </div>`;
}
...
export default async function handler(req, res) {
  ...
  const { to = [], subject, title, body, requestTitle, statusLabel, actorName } = req.body || {};
  const recipients = Array.isArray(to) ? to : String(to || "").split(",");
  const cleanRecipients = [...new Set(recipients.map((x) => String(x).trim()).filter(Boolean))];
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: buildHtml({ title, body, requestTitle, statusLabel, actorName }),
    });
